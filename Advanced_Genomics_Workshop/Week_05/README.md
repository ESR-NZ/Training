...
##### Getting the data
Note: The data is ~236 MB. To download the data, please run these commands:
...
To begin, create a directory called `raw_reads` and move all ten files into it. Afterwards, navigate to the `raw_reads` folder:
...
```bash
# Move all files ending in .fastq.gz into the raw_reads directory:
mv /path/to/*.fastq.gz raw_reads/.
```
